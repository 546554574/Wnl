...
open class JsonConvert<T> : Converter<T> {
    ...
    private fun parseParameterizedType(response: Response, type: ParameterizedType?): T? {
        ...
        if (rawType !== LzyResponse!!.javaClass) {
            ...
            if (typeArgument === Void::class.java) {
                ...
                val simpleResponse = Convert.fromJson<SimpleResponse<T>>(jsonReader, SimpleResponse::class.java)
                response.close()
                ...
                response.close()
                val code = lzyResponse.error_code
                //这里的0是以下意思
                ...
                if (code == 0) {
                    ...
                    throw IllegalStateException("错误代码：" + code + "，错误信息：" + lzyResponse.reason)
                }
            }
        }
    }
